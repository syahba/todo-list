  ...
  return (
    <div className="container">
      ...
      {tasks.activeTasks ? tasks.activeTasks.map(val => (
        <ActiveTask key={val.id} data={val.task}
          checkBtn={() => checkHandler(val.id, val.isCompleted)}
          deleteBtn={() => deleteHander(val.id)}></ActiveTask>
      )) : <div></div>}

      {/* run if completed tasks array is not empty */}
      {tasks.completedTasks ? tasks.completedTasks.map(val => (
        <CompletedTask key={val.id} data={val.task}
          checkBtn={() => checkHandler(val.id, val.isCompleted)}
          deleteBtn={() => deleteHander(val.id)}></CompletedTask>
      )) : <div></div>}
    </div>
  );
}
